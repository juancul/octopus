...
class WooCommerceCheckOutForm extends FormHandler implements IFormHandler
{
    ...
    function createphonenumberdatabasetable()
    {
        global $wpdb;

        $table_name = $wpdb->prefix . "mo_first_checkout";
    ...
    function handleForm()
    {
        // $content = MoUtility::send_phone_notif($phone_number,$message);
        ...
        $this->createphonenumberdatabasetable();
        ...
        add_action("wp_ajax_is_first_order",array($this, 'is_first_order' ));
        add_action("wp_ajax_nopriv_is_first_order",array($this, 'is_first_order' ));
    ...
    function is_first_order()
    {
        ...
        $user_order_count = wc_get_customer_order_count( get_current_user_id());

        if ($user_order_count==0) 
        {
            $_SESSION['is_first_order']=true;
            echo json_encode(['result'=>'success','message'=>'is_first_order_true']);
            die();
        }
        else
        {
            $_SESSION['is_first_order']=false;
            
            echo json_encode(['result'=>'success','message'=>'is_first_order_false']);
            die();
        }
    ...
    /*function is_first_order()
    {
        ...
        $getphonenumber=$wpdb->get_results("SELECT * FROM `{$wpdb->prefix}mo_first_checkout` WHERE `phone_number`='$phonenumber'");
        
        if (count($getphonenumber)==0) 
        {
            $_SESSION['is_first_order']=true;
            echo json_encode(['result'=>'success','message'=>'is_first_order_true']);
            die();
    ...
    function add_custom_popup()
{
    ...
    function _showButtonOnPage()
    {
        if(strcasecmp($this->_otpType,$this->_typePhoneTag)==0)
            echo '<input type="button" class="button alt" style="'
                . ( $this->_popupEnabled ? 'float: right;line-height: 1;width:100%;margin-right: 2em;padding: 1em 2em; display:none;' : 'display:none;width:100%;margin-bottom: 15px;' )
    ...
    function handle_post_verification($redirect_to,$user_login,$user_email,$password,$phone_number,$extra_data,$otpType)
    {
        ...
        global $wpdb;
        $table_name = $wpdb->prefix . "mo_first_checkout";
        $wpdb->insert($table_name, array('phone_number' => $phone_number,) );
        $_SESSION['is_first_order']=false;
    ...
    function handleFormOptions()
    {
        ...
        $this->redirect_url = $this->sanitizeFormPOST('custom_messages_redirect_url');
        ...
        if($this->basicValidationCheck(BaseMessages::WC_CHECKOUT_CHOOSE)) {
            ...
            update_mo_option('custom_messages_redirect_url', $this->redirect_url);
    ...
    {
        return get_mo_option("custom_messages_redirect_url");
    }

    ...
    public function apply_status( $order, $transaction ) {
        switch ( $transaction->status ) {
            case WC_Wompi_API::STATUS_APPROVED:
                ...
                $this->update_transaction_status( $order, __('Wompi payment APPROVED. TRANSACTION ID: ', 'woocommerce-gateway-wompi'));
                break;
            ...
            case WC_Wompi_API::STATUS_DECLINED:
                $this->update_transaction_status( $order, __('Wompi payment DECLINED. TRANSACTION ID: ', 'woocommerce-gateway-wompi') . ' (' . $transaction->id . ')', 'pending' );
                break;
            default : // ERROR
                $this->update_transaction_status( $order, __('Wompi payment ERROR. TRANSACTION ID: ', 'woocommerce-gateway-wompi') . ' (' . $transaction->id . ')', 'pending' );
        }
    }

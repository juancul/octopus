...
if (!class_exists('VG_Freemium_Plugin_SDK')) {
	...
	class VG_Freemium_Plugin_SDK {
		...
		function admin_enqueue_scripts($hook) {
			...
			$allowed_pages = apply_filters('vg_plugin_sdk/assets/allowed_pages', array(
		...
		function redirect_to_welcome_page() {
			...
			$url = ( ! empty($this->settings['welcome_page_url'])) ? $this->settings['welcome_page_url'] : add_query_arg(array('page' => $this->settings['plugin_prefix'] . 'welcome_page'), admin_url('admin.php'));
			wp_redirect($url);
			exit();
		}
